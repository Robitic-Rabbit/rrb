...
async function main() {
  try {
    ...
    const mintingSpecialNumber = process.env._MINTING_SPECIAL;
    console.log("mintingWeaponNumber : " + mintingSpecialNumber);

    const provider = ethers.provider;
    const feeData = await provider.getFeeData(); // Get gas fee data

    /*const max_FeePerGas = feeData.maxFeePerGas ? feeData.maxFeePerGas * BigInt(2) : undefined;
    console.log("max_FeePerGas : " + max_FeePerGas);

    const maxPriorityFeePerGas = feeData.maxPriorityFeePerGas ? feeData.maxPriorityFeePerGas * BigInt(2) : undefined;
    console.log("maxPriorityFeePerGas : " + maxPriorityFeePerGas);*/

    const weaponContract = await ethers.getContractAt(ABI, "0x94e843871398a138a243974aA069bd08DD40f5E2");
    console.log("weaponContract : " + JSON.stringify(weaponContract));

    const tx = await weaponContract.mint(userAddress, mintingSpecialNumber, {
